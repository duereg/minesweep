...
(function(undefined) {
	...
	function addMinesToBoard(board, config, tileBuilder) {
		...
		for(var i=0; i < minePositions.length; i++) {
			...
			addMine(board, tileBuilder, mineSpot);
		}

		board.minePositions = minePositions;
	}

	function addMine(board, tileBuilder, position ) {
		board[position.column][position.row] = tileBuilder.createMine(position.column, position.row);
		incrementNeighbors(board, tileBuilder, position);
	}
