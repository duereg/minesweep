...
(function(undefined) {
	...
	function incrementNeighbors(board, tileBuilder, mineColumn, mineRow) {
		incrementNeighbor(board, tileBuilder, mineColumn - 1, mineRow);
		incrementNeighbor(board, tileBuilder, mineColumn + 1, mineRow);
		incrementNeighbor(board, tileBuilder, mineColumn, mineRow - 1);
		incrementNeighbor(board, tileBuilder, mineColumn, mineRow + 1);

		incrementNeighbor(board, tileBuilder, mineColumn - 1, mineRow - 1);
		incrementNeighbor(board, tileBuilder, mineColumn + 1, mineRow + 1);
		incrementNeighbor(board, tileBuilder, mineColumn + 1, mineRow - 1);
		incrementNeighbor(board, tileBuilder, mineColumn - 1, mineRow + 1);
	}
	...
	function getMinePositions(config) {
	...
	function addMines(board, config, tileBuilder, minePlacements) {
		if( !!!minePlacements) throw "invalid minePlacements";

		while(minePlacements.length > 0) {
			var mineSpot = minePlacements.shift();
			var mineRow = Math.floor(mineSpot / config.columns) + 1;
			var mineColumn = mineSpot % config.columns + 1;

			board[mineColumn][mineRow] = tileBuilder.createMine(mineColumn, mineRow);
			incrementNeighbors(board, tileBuilder, mineColumn, mineRow);
		}
	}
	...
	var builder = {
		newGame: function(config, tileBuilder) {
			...
			var mineLocations = getMinePositions(config);
			this.addTilesToBoard(board, tileBuilder);
			addMines(board, config, tileBuilder, mineLocations);
			return board;
		},
