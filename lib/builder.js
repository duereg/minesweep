...
(function(undefined) {
	...
	function addMines(board, tileBuilder, minePlacements) {
		...
		while(minePlacements.length > 0) {
			...
			board[mineColumn][mineRow] = tileBuilder.addMine(mineColumn, mineRow);
	...
	var builder = {
		newGame: function(config, tileBuilder) {
			...
			addMines(board, tileBuilder, mineLocations);
			return board;
		},
